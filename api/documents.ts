...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!q) {
    return res.json({ count: DOCUMENTS.length, documents: DOCUMENTS.slice(0, 50) })
  }

  const matches = DOCUMENTS.filter((d) => matchesQuery(d, q))
  return res.json({ count: matches.length, documents: matches })
}
